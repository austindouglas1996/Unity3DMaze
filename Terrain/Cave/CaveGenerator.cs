...
public class CaveGenerator : MonoBehaviour
{
    ...
    public float[,,] UpdateDensityMap(Vector3Int chunkPos)
    {
        return SolidifyEdgesFromNeighbors(chunkPos, Chunks[chunkPos].DensityMap, neighborPos => Chunks.TryGetValue(neighborPos, out var neighborMap) ? neighborMap : null);
    }

    private float[,,] SolidifyEdgesFromNeighbors(Vector3Int chunkPos, float[,,] densityMap, System.Func<Vector3Int, CaveChunk> getNeighborDensity)
    {
        ...
        // +X face
        {
            Vector3Int neighborPos = chunkPos + Vector3Int.right;
            float[,,] neighbor = getNeighborDensity(neighborPos).DensityMap;
            if (neighbor != null)
            {
                for (int y = 0; y < sizeY; y++)
                {
                    ...
                    {
                        float a = neighbor[0, y, z];
                        float b = densityMap[sizeX - 1, y, z];
                        if (a < threshold || b < threshold)
                        {
                            densityMap[sizeX - 1, y, z] = threshold;
                        }
                    }
                }
            }
        ...
        // -X face
        {
            Vector3Int neighborPos = chunkPos + Vector3Int.left;
            float[,,] neighbor = getNeighborDensity(neighborPos).DensityMap;
            if (neighbor != null)
            {
                for (int y = 0; y < sizeY; y++)
                {
                    for (int z = 0; z < sizeZ; z++)
                    {
                        if (neighbor[neighbor.GetLength(0) - 1, y, z] < threshold && densityMap[0, y, z] < threshold)
                        {
                            densityMap[0, y, z] = threshold;
                        }
                    }
                }
            }
        ...
        // +Y face
        {
            Vector3Int neighborPos = chunkPos + Vector3Int.up;
            float[,,] neighbor = getNeighborDensity(neighborPos).DensityMap;
            if (neighbor != null)
            {
                for (int x = 0; x < sizeX; x++)
                {
                    for (int z = 0; z < sizeZ; z++)
                    {
                        if (neighbor[x, 0, z] < threshold && densityMap[x, sizeY - 1, z] < threshold)
                        {
                            densityMap[x, sizeY - 1, z] = threshold;
                        }
                    }
                }
            }
        ...
        // -Y face
        {
            Vector3Int neighborPos = chunkPos + Vector3Int.down;
            float[,,] neighbor = getNeighborDensity(neighborPos).DensityMap;
            if (neighbor != null)
            {
                for (int x = 0; x < sizeX; x++)
                {
                    for (int z = 0; z < sizeZ; z++)
                    {
                        if (neighbor[x, neighbor.GetLength(1) - 1, z] < threshold && densityMap[x, 0, z] < threshold)
                        {
                            densityMap[x, 0, z] = threshold;
                        }
                    }
                }
            }
        ...
        // +Z face
        {
            Vector3Int neighborPos = chunkPos + Vector3Int.forward;
            float[,,] neighbor = getNeighborDensity(neighborPos).DensityMap;
            if (neighbor != null)
            {
                for (int x = 0; x < sizeX; x++)
                {
                    for (int y = 0; y < sizeY; y++)
                    {
                        if (neighbor[x, y, 0] < threshold && densityMap[x, y, sizeZ - 1] < threshold)
                        {
                            densityMap[x, y, sizeZ - 1] = threshold;
                        }
                    }
                }
            }
        ...
        // -Z face
        {
            Vector3Int neighborPos = chunkPos + Vector3Int.back;
            float[,,] neighbor = getNeighborDensity(neighborPos).DensityMap;
            if (neighbor != null)
            {
                for (int x = 0; x < sizeX; x++)
                {
                    for (int y = 0; y < sizeY; y++)
                    {
                        if (neighbor[x, y, neighbor.GetLength(2) - 1] < threshold && densityMap[x, y, 0] < threshold)
                        {
                            densityMap[x, y, 0] = threshold;
                        }
                    }
                }
            }
